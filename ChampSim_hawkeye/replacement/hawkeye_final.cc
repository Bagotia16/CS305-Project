...
#define OPTGEN_VECTOR_SIZE 1024
#include "optgen.h"
...
void CACHE::InitReplacementState()
{
...
uint32_t CACHE::GetVictimInSet(uint32_t cpu, uint32_t set, const BLOCK *current_set, uint64_t PC, uint64_t paddr, uint32_t type)
{   
    //cout<<"-----------------------------------------------"<<endl;
...
void CACHE::replace_addr_history_element(unsigned int sampler_set)
{
    //cout << "here--------------------------------------------here" << endl;
    uint64_t lru_addr = 0;
...
void CACHE::update_addr_history_lru(unsigned int sampler_set, unsigned int curr_lru)
{
    for (map<uint64_t, ADDR_INFO>::iterator it = addr_history[sampler_set].begin(); it != addr_history[sampler_set].end(); it++)
    {
...
void CACHE::UpdateReplacementState(uint32_t cpu, uint32_t set, uint32_t way, uint64_t paddr, uint64_t PC, uint64_t victim_addr, uint32_t type, uint8_t hit)
{
    paddr = (paddr >> 6) << 6;
...
void CACHE::PrintStats_Heartbeat()
{
}
